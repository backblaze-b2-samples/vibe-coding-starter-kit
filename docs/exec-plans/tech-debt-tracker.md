...
| Byte-exact OpenAPI comparison against floating Python deps (`requirements.txt` is `>=` only, no lockfile) | A FastAPI/Pydantic release that changes schema generation turns `verify-api` red on every PR with no repo change; the failure reads as "artifact is stale" and points at the wrong cause | Pin `fastapi`/`pydantic` (or add a lockfile) so contract output only moves on a deliberate bump, or compare a normalized projection (paths, verbs, schema names) instead of exact bytes | Medium |
